...
class EventsRepositorySQLImpl(EventsRepo):
    ...
    def setup(self):
        try:
            if not SQLiteHandler.check_if_table_exists(
                self.db_name, self.table_name
            ):
                SQLiteHandler.create_table(self.db_name, self.table_name)
    ...
    def add(self, new_event: Event):
        ...
        self.notify_observers(
            method=RepoMethod.CREATE,
            details=RepoActionDetails(
                event_id=new_event.event_id, event=item_to_insert
            ),
        )
