...
class TestEventsRoutes:
    ...
    @pytest.mark.parametrize(
        "key",
        (SortKey.DATE, SortKey.NUMBER_OF_PARTICIPANTS, SortKey.CREATION_TIME),
    )
    def test_sort_events(self, key: SortKey):
        ...
        match key:
            case SortKey.DATE:
                events_copy.sort(
                    key=lambda x: x.event_time.value, reverse=True
                )
            ...
            case SortKey.CREATION_TIME:
                events_copy.sort(
                    key=lambda x: x.creation_time.value, reverse=True
                )
    ...
    def test_get_all_events(self, client, mocker):
        ...
        assert response.status_code == 200 and content["message"] == [
            self.expected_returns_event(event)
            for event in self.test_repo.get_all()
        ]
    ...
    def test_update_event(self, client, mocker):
        ...
        assert (
            ...
            and updated_event.event_time.value
            == datetime.fromisoformat("2600-12-10T10:51:36.347").replace(
                tzinfo=timezone.utc
            )
